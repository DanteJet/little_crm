...
export function login({ error = '' }) {
  return layout({ title: 'Вход', user: null, body: `<section class="auth card"><h1>Вход</h1>${error ? `<p class="alert">${html(error)}</p>` : ''}<form method="post" action="/login" class="form"><label>ФИО<input name="full_name" autocomplete="name" required></label><label>Логин<input name="login" autocomplete="username" required></label><label>Пароль<input name="password" type="password" autocomplete="current-password" required></label><button>Войти</button></form><p class="hint">Демо-админ: <code>admin</code> / <code>admin123</code>. Смените пароль через переменные окружения при первом запуске.</p></section>` });
}
